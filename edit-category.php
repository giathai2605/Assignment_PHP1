...
$data = executeQuery($sqlQuery,true); //Lấy về tất cả các bản ghi từ câu sql

// $data = $stmt->fetch();  - Lấy ra 1 bản ghi đầu tiên từ câu sql  
$id=$_GET['id'];
$index=[];
...
<body>
<div class="container">
    <form id="contactus" action="save-edit-category.php?id=<?= $id ?>" method="post">
        <h3>Edit Category</h3>
        <fieldset> <input placeholder="name" name="name" type="text" tabindex="1" required autofocus value="<?= $index[1]?>"> 
        <br>
        <?php if(isset($_GET['name_err'])):?>
                <span style="color: red"><?= $_GET['name_err']?></span>
            <?php endif ?></fieldset></fieldset>
